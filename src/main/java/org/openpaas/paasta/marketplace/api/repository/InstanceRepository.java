...
public interface InstanceRepository extends JpaRepository<Instance, Long>, JpaSpecificationExecutor<Instance> {
	...
	@Query(value="SELECT  ROUND(SUM((IFNULL(sp.cpu_amt,0)+IFNULL(sp.memory_amt,0)+IFNULL(sp.disk_amt,0))/cl.days)) AS usagePriceTotal \n"
				...
				+"WHERE   1=1 \n"
				+"AND     cl.dt BETWEEN IF(DATE_FORMAT(it.usage_start_date, '%Y%m%d') >= DATE_FORMAT(:usageStartDate, '%Y%m%d') \n"
				+"                        ,DATE_FORMAT(it.usage_start_date, '%Y%m%d') \n"
				+"                        ,DATE_FORMAT(:usageStartDate, '%Y%m%d')) \n"
				+"                  AND IF(DATE_FORMAT(IFNULL(it.usage_end_date, :usageEndDate), '%Y%m%d') <= DATE_FORMAT(:usageEndDate, '%Y%m%d') \n"
				+"                        ,DATE_FORMAT(IFNULL(it.usage_end_date, :usageEndDate), '%Y%m%d') \n"
				+"                        ,DATE_FORMAT(:usageEndDate,'%Y%m%d')) \n"
